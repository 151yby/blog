...
export const useMusicStore = defineStore('music', {
  ...
  actions: {
    ...
    async fetchMusicUrl(musicId) {
      try {
        const response = await axios.post('api/download_music/', { music_id: musicId, ablum_cover: ablumcover});
        const musicUrl = response.data.music_url;
        const albumUrl = response.data.album_url;
        // 更新对应歌曲的 src
        const songIndex = this.playlist.findIndex(song => song.music_id === musicId);
        if (songIndex !== -1) {
          this.playlist[songIndex].src = musicUrl;
        }
        const albumsrc = this.playlist.findIndex(song => song.ablum_cover === ablumcover);
        if (albumsrc !== -1) {
          this.playlist[albumsrc].src = albumUrl;
        }
        return musicUrl;
      } catch (error) {
